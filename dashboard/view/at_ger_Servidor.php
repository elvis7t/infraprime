					...
					<div class="box box-primary">
									...
									<div class="form-group  col-md-2">  
									  <label for="mq_ip">#IP:</label> 
										<div class="input-group">
											...
											<input type="text"  class="form-control" name="mq_ip" id="mq_ip" value="<?=$mq_ip;?>"/>  
										</div>
									</div> 								 	
									<!-- /.col -->
									<div class="form-group col-md-2">
								...
								<div class="row">
									...
									<div class="form-group col-md-1"><!--Tem que ser antes do selectt -->  
									  <label for="mq_licenca">Aloca&ccedil;&atilde;o</label><br>  
										<input type="radio" class="minimal" value=f <?=($rs->fld("mq_servtp")==f?"CHECKED":"");?> id="mq_servtp" name="mq_servtp"> Fisico<br>
										<input type="radio" class="minimal" value=v <?=($rs->fld("mq_servtp")==v?"CHECKED":"");?> id="mq_servtp" name="mq_servtp"> Virtual<br>										
									</div>
								
									<!-- /.col -->									
									<?php if($rs->fld("mq_servtp")==v): ?>					
									<div class="form-group col-md-1"><!--Tem que ser antes do selectt -->  
									  <label for="mq_licenca">Cluster</label><br>  
										<input type="radio" class="minimal" value=1 <?=($rs->fld("mq_servcluster")==1?"CHECKED":"");?> id="mq_servcluster" name="mq_servcluster"> Sim<br>
										<input type="radio" class="minimal" value=0 <?=($rs->fld("mq_servcluster")==0?"CHECKED":"");?> id="mq_servcluster" name="mq_servcluster"> Nao<br>
										<input type="hidden" id="mq_nf" name="mq_nf" value="<?=$mq_nf?>"/>
									</div>
									<?php else: ?>
									<div class="form-group col-md-2">
									...
									<div class="form-group col-md-5">   
									  <label for="mq_licenca">Baixar Nota Fiscal</label><br> 
										<a class="btn-lg primary " data-toggle='tooltip' data-placement='bottom' title='Baixar Nota <?=$mq_nf;?>'   href="../images/nota_fiscal/download.php?arquivo=<?=$mq_nota_fiscal;?>.pdf"><i class="fa fa-cloud-download fa-3x"></i></a>																															
									</div>	
									<?php endif; ?>	
									<div class="col-md-12">
										<div class="box box-primary collapsed-box box-solid">
											<div class="box-header with-border">
											  <h3 class="box-title"><i class="fa fa-server"></i>  Observações </h3>
											...
											<div class="box-body" >
											  ...
											  <textarea   class="form-control" id="mq_servobs" name="mq_servobs" ><?=$mq_servobs;?></textarea>
											</div>
											<!-- /.box-body -->
										</div>
										<!-- /.box -->
						...
						<div class="box-footer">
							...
							<table id="maquinas" class="table table-bordered table-striped">     
								<tr> 
									<td>
										<button class="<?=$menu[$acao]['class'];?>" type="button" data-toggle='tooltip' data-placement='bottom' title='Alterar Servidor' id="<?=$menu[$acao]['id'];?>"><i class="<?=$menu[$acao]['icone'];?>" ></i> <?=$menu[$acao]['label'];?></button>
									</td> 
									<td>
									...
									<?php if($rs->fld("mq_servtp")==f): ?>
									<td>
										<a 	class="btn btn-sm btn-warning" data-toggle='tooltip' data-placement='bottom' title='Emprestimo'  a href="at_empre_Servidor.php?token=<?=$_SESSION['token']?>&acao=N&mqid=<?=$mq_id;?>"><i class="glyphicon glyphicon-retweet"></i> Emprestimo</a>
									</td>
									<td>
										<a 	class="btn btn-sm btn-danger" data-toggle='tooltip' data-placement='bottom' title='Manuten&ccedil;&atilde;o'  a href="at_man_Servidor.php?token=<?=$_SESSION['token']?>&acao=N&mqid=<?=$mq_id?>"><i class="fa fa-wrench"></i> Manuten&ccedil;&atilde;o</a>
									</td>
									<td>
										<button type="button" id="btn_qrcodeSR" class="btn btn-sm btn-success" data-toggle='tooltip' data-placement='bottom' title='Gerar QRCODE' type="submit"><i class="fa fa-qrcode"></i> QRCODE</button>
									</td>
									<td>
										<a 	class="btn btn-sm btn-danger" data-toggle='tooltip' data-placement='bottom' title='Descarte'  a href="at_descartar_Servidor.php?token=<?=$_SESSION['token']?>&acao=N&mqid=<?=$mq_id?>"><i class="fa fa-recycle"></i> Descartar</a>
									</td>
									<td>
										<a 	class="btn btn-sm btn-info" data-toggle='tooltip' data-placement='bottom' title='Outras ocorrencias'  a href="at_outros_Mq.php?token=<?=$_SESSION['token']?>&acao=N&mqid=<?=$mq_id?>"><i class="glyphicon glyphicon-question-sign"></i> Outros</a>
									</td> 
									<td>
										<a 	class="btn btn-sm btn-primary" data-toggle='tooltip' data-placement='bottom' title='Atribuir Nota'  a href="at_upload_nota_fiscal_serv.php?token=<?=$_SESSION['token']?>&acao=N&mqid=<?=$mq_id;?>"><i class="fa fa-file-pdf-o"></i> Nota</a>
									</td>
									<?php else: ?>								
									<td>
										<button class="btn btn-sm btn-danger" type="button" id="btn_Desativar_serv"><i class="fa fa-power-off"></i> Desativar</a>										
									</td> 
									<?php endif; ?>
								</tr>
							</table>							
